...
def center_crop_affine(
    source_size: tuple[int, int],
    target_size: tuple[int, int],
) -> np.ndarray:
    """Map full-FOV source pixels into a centered CROP output."""
    ...
    crop_x = 0.5 * (scale * source_width - target_width)
    crop_y = 0.5 * (scale * source_height - target_height)
    return np.array(
        [
            [scale, 0.0, -crop_x],
            [0.0, scale, -crop_y],
...
def map_pixel_center_crop(
    pixel_uv: tuple[float, float] | list[float] | np.ndarray,
    source_size: tuple[int, int],
    target_size: tuple[int, int],
) -> np.ndarray:
    pixel = np.asarray(pixel_uv, dtype=np.float64).reshape(2)
    mapped = center_crop_affine(source_size, target_size) @ np.array([pixel[0], pixel[1], 1.0])
    return mapped[:2]


def map_pixel_from_center_crop(
    pixel_uv: tuple[float, float] | list[float] | np.ndarray,
    source_size: tuple[int, int],
    target_size: tuple[int, int],
) -> np.ndarray:
    """Map a centered CROP output pixel back into the full-FOV source."""
    pixel = np.asarray(pixel_uv, dtype=np.float64).reshape(2)
    mapped = np.linalg.inv(center_crop_affine(source_size, target_size)) @ np.array(
        [pixel[0], pixel[1], 1.0],
        dtype=np.float64,
    )
    return mapped[:2]
...
def lens_position_for_depth(
    ...
    macro_position: int = 255,
    macro_distance_m: float = 0.08,
) -> int:
    """Approximate VCM position assuming lens power is linear in inverse distance."""
    if not np.isfinite(depth_m) or depth_m <= 0.0:
        raise ValueError("focus depth must be finite and positive")
    macro_distance = max(1.0e-3, float(macro_distance_m))
    position = infinity_position + (
        (macro_position - infinity_position) * macro_distance / max(float(depth_m), macro_distance)
    )
    return int(round(np.clip(position, min(infinity_position, macro_position), max(infinity_position, macro_position))))


def source_camera_matrix_from_center_crop(
    target_camera_matrix: np.ndarray,
    source_size: tuple[int, int],
    target_size: tuple[int, int],
) -> np.ndarray:
    """Recover full-FOV intrinsics from intrinsics of a centered crop."""
    target_camera_matrix = np.asarray(target_camera_matrix, dtype=np.float64).reshape(3, 3)
    affine = center_crop_affine(source_size, target_size)
    return np.linalg.inv(affine) @ target_camera_matrix
